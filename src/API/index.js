...
//This fetch gets all users and then finds the username that w
// was passed in
export const getAllUsers = async (username) => {
  try {
    const response = await fetch(`${API_URL}/users`);
    const result = await response.json();
    //finds user where username is the same as the username
    //that is passed in
...
//This fetch gets the user which matches the ID that was passed
// in   returns the products for that user.
